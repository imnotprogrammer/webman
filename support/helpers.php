...
/**
 * 发送消息到队列，这个函数针对于stomp客户端
 * @param $queue
 * @param $data
 */
function send_message_to_queue($queue, $data, $connectionName = 'default') {
    $client = \support\stomp\Client::connection($connectionName);
    $client->send($queue, json_encode($data));
}
